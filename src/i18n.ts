...
onDeviceOff|Turn on on-device AI in Settings to use AI hints.|فعّل الذكاء على الجهاز في الإعدادات لاستخدام تلميحات الذكاء الاصطناعي.|Activa la IA en el dispositivo en Ajustes para usar pistas de IA.|Activez l’IA sur l’appareil dans les réglages pour utiliser les indices IA.|Aktiviere KI auf dem Gerät in den Einstellungen, um KI-Hinweise zu nutzen.|Ative a IA no dispositivo nas Definições para usar dicas de IA.|Attiva l’IA sul dispositivo nelle impostazioni per usare gli indizi IA.|Schakel AI op het apparaat in bij instellingen om AI-aanwijzingen te gebruiken.|Включите ИИ на устройстве в настройках, чтобы использовать подсказки ИИ.|Ayarlardan cihaz üzerinde yapay zekâyı açın, yapay zekâ ipuçlarını kullanmak için.|सेटिंग्स में डिवाइस पर AI चालू करें, AI संकेतों के लिए।|設定でオンデバイス AI を有効にすると、AI ヒントが使えます。|설정에서 온디바이스 AI를 켜면 AI 힌트를 사용할 수 있습니다.|在设置中开启端侧 AI 即可使用 AI 提示。|Aktifkan AI di perangkat di Pengaturan untuk memakai petunjuk AI.|سیٹنگز میں ڈیوائس پر AI آن کریں تاکہ AI اشارے استعمال کر سکیں۔
onDeviceWebGpu|Requires WebGPU. Try a recent browser with hardware acceleration enabled.|يتطلب WebGPU. جرّب متصفحًا حديثًا مع تسريع العتاد مفعّلًا.|Requiere WebGPU. Prueba un navegador reciente con la aceleración por hardware activada.|Nécessite WebGPU. Essayez un navigateur récent avec l’accélération matérielle activée.|Erfordert WebGPU. Nutze einen aktuellen Browser mit aktivierter Hardwarebeschleunigung.|Requer WebGPU. Experimente um navegador recente com a aceleração de hardware ativada.|Richiede WebGPU. Prova un browser recente con l'accelerazione hardware attivata.|Vereist WebGPU. Probeer een recente browser met hardwareversnelling ingeschakeld.|Требуется WebGPU. Попробуйте свежий браузер с включённым аппаратным ускорением.|WebGPU gerekir. Donanım hızlandırması açık olan güncel bir tarayıcı deneyin.|WebGPU आवश्यक है। हार्डवेयर एक्सेलरेशन चालू नया ब्राउज़र आज़माएँ।|WebGPU が必要です。ハードウェアアクセラレーションを有効にした新しいブラウザをお試しください。|WebGPU가 필요합니다. 하드웨어 가속이 켜진 최신 브라우저를 사용해 보세요.|需要 WebGPU。请尝试已开启硬件加速的新版浏览器。|Memerlukan WebGPU. Coba peramban terbaru dengan akselerasi perangkat keras aktif.|WebGPU درکار ہے۔ ہارڈ ویئر ایکسیلریشن آن رہنے والا حالیہ براؤزر آزمائیں۔
onDeviceReady|Model ready. Hints are now written on this device.|النموذج جاهز. تُكتب التلميحات الآن على هذا الجهاز.|Modelo listo. Las pistas ahora se escriben en este dispositivo.|Modèle prêt. Les indices sont désormais rédigés sur cet appareil.|Modell bereit. Hinweise werden jetzt auf diesem Gerät erstellt.|Modelo pronto. As dicas agora são escritas neste dispositivo.|Modello pronto. Gli indizi ora vengono scritti su questo dispositivo.|Model klaar. Aanwijzingen worden nu op dit apparaat geschreven.|Модель готова. Подсказки теперь создаются на этом устройстве.|Model hazır. İpuçları artık bu cihazda yazılıyor.|मॉडल तैयार है। संकेत अब इस डिवाइस पर लिखे जाते हैं।|モデルの準備ができました。ヒントはこの端末上で生成されます。|모델이 준비되었습니다. 이제 힌트가 이 기기에서 생성됩니다.|模型已就绪。提示现在由这台设备生成。|Model siap. Petunjuk kini ditulis di perangkat ini.|ماڈل تیار ہے۔ اشارے اب اس ڈیوائس پر لکھے جاتے ہیں۔
onDeviceDownload|Download the model (about 1.1 GB)|نزّل النموذج (نحو 1.1 غيغابايت)|Descargar el modelo (unos 1,1 GB)|Télécharger le modèle (environ 1,1 Go)|Modell herunterladen (etwa 1,1 GB)|Baixar o modelo (cerca de 1,1 GB)|Scarica il modello (circa 1,1 GB)|Model downloaden (ongeveer 1,1 GB)|Скачать модель (около 1,1 ГБ)|Modeli indir (yaklaşık 1,1 GB)|मॉडल डाउनलोड करें (लगभग 1.1 जीबी)|モデルをダウンロード（約 1.1 ギガバイト）|모델 다운로드 (약 1.1기가바이트)|下载模型（约 1.1 吉字节）|Unduh model (sekitar 1,1 GB)|ماڈل ڈاؤن لوڈ کریں (تقریباً 1.1 جی بی)
onDeviceAi|On-device AI|ذكاء على الجهاز|IA en el dispositivo|IA sur l’appareil|KI auf dem Gerät|IA no dispositivo|IA sul dispositivo|AI op het apparaat|ИИ на устройстве|Cihaz üzerinde yapay zekâ|डिवाइस पर AI|オンデバイス AI|온디바이스 AI|端侧 AI|AI di perangkat|آئی ڈیوائس پر
onDeviceDesc|Runs a small language model in this browser to write hints locally. It downloads once, works offline afterwards, and your puzzles never leave this device.|يشغّل نموذجًا لغويًا صغيرًا داخل هذا المتصفح ليكتب التلميحات محليًا. يُنزَّل مرة واحدة، ثم يعمل دون اتصال، وتبقى ألغازك على هذا الجهاز.|Ejecuta un modelo de lenguaje pequeño en este navegador para escribir pistas de forma local. Se descarga una vez, funciona sin conexión después y tus puzzles nunca salen de este dispositivo.|Exécute un petit modèle de langage dans ce navigateur pour rédiger les indices localement. Il se télécharge une fois, fonctionne ensuite hors ligne, et vos énigmes ne quittent jamais cet appareil.|Führt ein kleines Sprachmodell in diesem Browser aus, um Hinweise lokal zu schreiben. Einmaliger Download, danach offline nutzbar, und deine Rätsel verlassen dieses Gerät nie.|Executa um pequeno modelo de linguagem neste navegador para escrever dicas localmente. Baixa uma vez, depois funciona offline, e seus puzzles nunca saem deste dispositivo.|Esegue un piccolo modello linguistico in questo browser per scrivere gli indizi in locale. Si scarica una volta, poi funziona offline e i tuoi puzzle non lasciano mai questo dispositivo.|Draait een klein taalmodel in deze browser om aanwijzingen lokaal te schrijven. Eén keer downloaden, daarna offline werken, en je puzzels verlaten dit apparaat nooit.|Запускает небольшую языковую модель в этом браузере, чтобы подсказки создавались локально. Скачивается один раз, дальше работает офлайн, а ваши задания не покидают это устройство.|İpuçlarını yerel olarak yazmak için bu tarayıcıda küçük bir dil modeli çalıştırır. Bir kez indirilir, sonra çevrimdışı çalışır ve bulmacaların bu cihazdan çıkmaz.|संकेत स्थानीय रूप से लिखने के लिए इस ब्राउज़र में एक छोटा भाषा मॉडल चलाता है। यह एक बार डाउनलोड होता है, फिर ऑफ़लाइन काम करता है, और आपकी पहेलियाँ इस डिवाइस से बाहर नहीं जातीं।|ヒントを端末上で書くために、このブラウザ内で小さな言語モデルを実行します。一度ダウンロードすればその後はオフラインで使え、パズルがこの端末の外に出ることはありません。|힌트를 로컬에서 작성하도록 이 브라우저에서 작은 언어 모델을 실행합니다. 한 번 다운로드하면 이후에는 오프라인으로 동작하며, 퍼즐 데이터는 기기를 벗어나지 않습니다.|在此浏览器中运行一个小型语言模型来本地生成提示。只需下载一次，之后即可离线使用，你的谜题不会离开这台设备。|Menjalankan model bahasa kecil di peramban ini untuk menulis petunjuk secara lokal. Diunduh sekali, lalu berfungsi luring, dan teka-tekamu tidak pernah meninggalkan perangkat ini.|اشارے مقامی طور پر لکھنے کے لیے اس براؤزر میں ایک چھوٹا زبانی ماڈل چلاتا ہے۔ ایک بار ڈاؤن لوڈ ہوتا ہے، پھر آف لائن کام کرتا ہے، اور آپ کے پزلز کبھی اس ڈیوائس سے باہر نہیں جاتے۔
badEmail|That does not look like an email address.|لا يبدو عنوان بريد إلكتروني.|Eso no parece una dirección de correo electrónico.|Cela ne ressemble pas à une adresse e-mail.|Das sieht nicht nach einer E-Mail-Adresse aus.|Isso não parece um endereço de e-mail.|Non sembra un indirizzo e-mail.|Dat lijkt geen e-mailadres te zijn.|Это не похоже на адрес электронной почты.|Bu bir e-posta adresine benzemiyor.|यह ईमेल पते जैसा नहीं लगता।|メールアドレスとして見えない形式です。|이메일 주소로 보이지 않습니다.|这看起来不像电子邮箱地址。|Itu tidak terlihat seperti alamat email.|یہ ای میل پتے کی طرح نہیں لگتا۔
signupBusy|Too many attempts. Try again in a few minutes.|محاولات كثيرة. حاول مجددًا بعد بضع دقائق.|Demasiados intentos. Inténtalo de nuevo en unos minutos.|Trop de tentatives. Réessayez dans quelques minutes.|Zu viele Versuche. In ein paar Minuten erneut versuchen.|Tentativas demais. Tente novamente em alguns minutos.|Troppi tentativi. Riprova tra qualche minuto.|Te veel pogingen. Probeer het over een paar minuten opnieuw.|Слишком много попыток. Повторите через несколько минут.|Çok fazla deneme. Birkaç dakika sonra tekrar dene.|बहुत अधिक प्रयास। कुछ मिनट बाद फिर से कोशिश करें।|試行回数が多すぎます。数分後に再度お試しください。|시도가 너무 많습니다. 몇 분 후 다시 시도하세요.|尝试次数过多。请几分钟后重试。|Terlalu banyak percobaan. Coba lagi dalam beberapa menit.|بہت زیادہ کوششیں۔ چند منٹ بعد دوبارہ کوشش کریں۔
signupBlocked|This game cannot create accounts right now. Please try again later.|لا يمكن للعبة إنشاء حسابات حاليًا. حاول لاحقًا.|Este juego no puede crear cuentas ahora. Inténtalo más tarde.|Ce jeu ne peut pas créer de comptes pour le moment. Réessayez plus tard.|Dieses Spiel kann gerade keine Konten erstellen. Bitte später versuchen.|Este jogo não pode criar contas agora. Tente mais tarde.|Questo gioco non può creare account ora. Riprova più tardi.|Deze game kan nu geen accounts aanmaken. Probeer het later.|Игра не может создавать аккаунты прямо сейчас. Попробуйте позже.|Bu oyun şu anda hesap oluşturamıyor. Lütfen daha sonra tekrar dene.|यह गेम अभी खाते नहीं बना सकता। कृपया बाद में कोशिश करें।|このゲームは現在アカウントを作成できません。しばらくしてからお試しください。|이 게임은 지금 계정을 만들 수 없습니다. 나중에 다시 시도해 주세요.|本游戏暂时无法创建账户。请稍后再试。|Permainan ini belum bisa membuat akun. Silakan coba lagi nanti.|یہ گیم اِس وقت اکاؤنٹ نہیں بنا سکتی۔ براہِ کرم بعد میں کوشش کریں۔
username|Username|اسم المستخدم|Nombre de usuario|Nom d’utilisateur|Benutzername|Nome de utilizador|Nome utente|Gebruikersnaam|Имя пользователя|Kullanıcı adı|उपयोगकर्ता नाम|ユーザー名|사용자 이름|用户名|Nama pengguna|صارف نام
usernameHint|3–24 characters. Letters, numbers, dot, underscore or hyphen.|3–24 حرفًا. أحرف أو أرقام أو نقطة أو شرطة سفلية أو شرطة.|3–24 caracteres. Letras, números, punto, guion bajo o guion.|3 à 24 caractères. Lettres, chiffres, point, tiret bas ou trait d’union.|3–24 Zeichen. Buchstaben, Ziffern, Punkt, Unterstrich oder Bindestrich.|3–24 caracteres. Letras, números, ponto, sublinhado ou hífen.|3–24 caratteri. Lettere, numeri, punto, underscore o trattino.|3–24 tekens. Letters, cijfers, punt, underscore of streepje.|3–24 символа. Буквы, цифры, точка, подчёркивание или дефис.|3–24 karakter. Harfler, rakamlar, nokta, alt çizgi veya tire.|3–24 वर्ण। अक्षर, अंक, बिंदु, अंडरस्कोर या हाइफ़न।|3〜24文字。英数字、ドット、アンダースコア、ハイフン。|3~24자. 문자, 숫자, 점, 밑줄, 하이픈.|3–24 个字符。字母、数字、点、下划线或连字符。|3–24 karakter. Huruf, angka, titik, garis bawah, atau tanda hubung.|3–24 حروف۔ حروف، ہندسیات، نقطہ، انڈر سکور یا ہائیفن۔
usernameTaken|That username is already taken in this mode.|اسم المستخدم هذا محجوز في هذه اللعبة بالفعل.|Ese nombre de usuario ya está en uso en este modo.|Ce nom d’utilisateur est déjà pris dans ce mode.|Dieser Benutzername ist in diesem Modus bereits vergeben.|Esse nome de utilizador já está em uso neste modo.|Questo nome utente è già in uso in questa modalità.|Deze gebruikersnaam is al in deze modus in gebruik.|Это имя пользователя уже занято в этом режиме.|Bu kullanıcı adı bu modda zaten alınmış.|यह उपयोगकर्ता नाम इस मोड में पहले से लिया जा चुका है।|このユーザー名は、このモードではすでに使われています。|이 사용자 이름은 이 모드에서 이미 사용 중입니다.|该用户名在此模式中已被使用。|Nama pengguna itu sudah dipakai di mode ini.|یہ صارف نام اس موڈ میں پہلے ہی استعمال ہو چکا ہے۔
badUsername|Pick a username of 3–24 characters: letters, numbers, dot, underscore or hyphen.|اختر اسم مستخدم من 3–24 حرفًا: أحرف أو أرقام أو نقطة أو شرطة سفلية أو شرطة.|Elige un nombre de usuario de 3–24 caracteres: letras, números, punto, guion bajo o guion.|Choisissez un nom d’utilisateur de 3 à 24 caractères : lettres, chiffres, point, tiret bas ou trait d’union.|Wähle einen Benutzernamen mit 3–24 Zeichen: Buchstaben, Ziffern, Punkt, Unterstrich oder Bindestrich.|Escolhe um nome de utilizador com 3–24 caracteres: letras, números, ponto, sublinhado ou hífen.|Scegli un nome utente di 3–24 caratteri: lettere, numeri, punto, underscore o trattino.|Kies een gebruikersnaam van 3–24 tekens: letters, cijfers, punt, underscore of streepje.|Выберите имя пользователя из 3–24 символов: буквы, цифры, точка, подчёркивание или дефис.|3–24 karakterlik bir kullanıcı adı seç: harfler, rakamlar, nokta, alt çizgi veya tire.|3–24 वर्णों का उपयोगकर्ता नाम चुनें: अक्षर, अंक, बिंदु, अंडरस्कोर या हाइफ़न।|ユーザー名は3〜24文字で指定してください: 英数字、ドット、アンダースコア、ハイフン。|사용자 이름은 3~24자여야 합니다: 문자, 숫자, 점, 밑줄, 하이픈.|请选择 3–24 个字符的用户名：字母、数字、点、下划线或连字符。|Pilih nama pengguna 3–24 karakter: huruf, angka, titik, garis bawah, atau tanda hubung.|3–24 حروف کا صارف نام منتخب کریں: حروف، ہندسیات، نقطہ، انڈر سکور یا ہائیفن۔
